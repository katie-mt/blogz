...
class User(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    #our users will have username and password properties
    #we shouldn't have two different accounts associated with a username.  unique=True ensures that SQLAlchemy will not allow the creation of two separate users with the same username. 
    ...
    #users should always have usernames and passwords so we'll add those as parameters to our constructor
...
def login():
    ...
    if request.method == 'POST':
        #remember that there are two inputs on the login form, one that is name=username and one that is name=password.  Get this data out of the request
        ##request.form is the python dictionary that contains Post data or data sent in a post request
        username = request.form['username']
        password = request.form['password']
        #try to verify the user's password.  Need to retrieve the user and the associated username from the database. 
        ...
        #this will return a user with the given username with the given username if the user exists.  If the user doesn't exist then the user variable will be equal to the special value none. 
        user = User.query.filter_by(username=username).first()
        #to verify that not only do i have the correct password but do I have a user in the system with the given username
        #this will check first, does the user exist.  If this is none (query did not return a user with the given username from the database) then this will be none and the conditional will short circuit.
...
def signup():
    ...
    #in the registration template we have a username, password and verify.  Retrieve those parameters from the database
        username = request.form['username']
        password = request.form['password']
        ...
        if not existing_user:
            ...
            db.session.commit()
            #we put this above under login as well.  Once a user registers and we put them in the database, we'll log them in by putting their username in the session.
